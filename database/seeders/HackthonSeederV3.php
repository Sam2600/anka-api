 ...
 *
 * Idempotent — wipes 'brycen-myanmar-v3', 'brycen-myanmar-v2', AND 'brycen-myanmar' (V1)
 * because they share the same email addresses.
 *
 *   php artisan db:seed --class=HackthonSeederV3
 */
class HackthonSeederV3 extends Seeder
{
    private const SLUG = 'brycen-myanmar-v3';
    ...
    private const SUPER_ADMIN_EMAIL = 'superadmin@anka.io';
    ...
    public function run(): void
    {
        Model::unguarded(function () {
            ...
            $this->command->info('Brycen Myanmar V3 tenant seeded.');
            $this->command->info('  Tenant ID: '.$tenant->id);
            $this->command->info('  Logins (password = '.self::PASSWORD.'):');
            $this->command->info('    '.str_pad('SuperAdmin', 10).' '.$superAdmin->email);
            foreach ($users as $u) {
    ...
    {
        User::withoutGlobalScopes()->where('email', self::SUPER_ADMIN_EMAIL)->forceDelete();

        $tenants = Tenant::whereIn('slug', [self::SLUG, 'brycen-myanmar-v2', 'brycen-myanmar'])->get();
    ...
    private function createWayneEnterprise(Tenant $tenant, array $employees, array $roles, User $admin, Carbon $yearStart, array $rankAvg): void
    {
        ...
        [$contract, $project] = $this->createContractAndProject($tenant, $deal, $admin, [
            'contract_number' => 'V3-CON-2026-001',
            'project_number' => 'V3-PRJ-2026-101',
    ...
    private function createLuxLuthor(Tenant $tenant, array $employees, array $roles, User $admin, Carbon $yearStart, array $rankAvg): void
    {
        $team = [
            ...
            ['emp' => $employees['member7'], 'role_code' => 'qa',       'feature' => 'QA automation + testing',       'hours_per_month' => 160, 'months' => 10],
        ];
        $start = Carbon::create(2026, 3, 1);
        $end = Carbon::create(2026, 12, 31);
        $months = 10;
        $totalHours = $months * 4 * 160; // 6400h

        // 1L+3M constant for 10 months
        $monthlyTeam = array_fill(0, $months, ['leaders' => 1, 'members' => 3]);
        $costs = $this->projectCosts($monthlyTeam, $rankAvg);
        ...
        [$contract, $project] = $this->createContractAndProject($tenant, $deal, $admin, [
            'contract_number' => 'V3-CON-2026-002',
            'project_number' => 'V3-PRJ-2026-102',
            'budget_hours' => $totalHours,
        ]);
    ...
    {
        $team = [
            ['emp' => $employees['leader3'], 'role_code' => 'pm',      'feature' => 'Tech lead',       'hours_per_month' => 160, 'months' => 5],
            ['emp' => $employees['member6'], 'role_code' => 'backend', 'feature' => 'Backend services', 'hours_per_month' => 160, 'months' => 5],
    ...
    private function createMonthlyInvoices(Tenant $tenant, Contract $contract, Carbon $start, array $rows, array $monthlyTeam, array $rankAvg): void
    {
        $recognized = 0;
        foreach ($rows as $idx => $row) {
            ...
            $paid = $isPaid ? $issue->copy()->addDays(5) : null;
            $billingMonth = $start->copy()->addMonths($row['offset']);

            $team = $monthlyTeam[$row['offset']];
            $lineItems = [];
            if ($team['leaders'] > 0) {
                $lineItems[] = [
                    'kind' => 'resource',
                    'label' => 'Project Manager (Lead)',
                    'quantity' => $team['leaders'],
                    'cost' => round($rankAvg['leader'], 2),
                    'amount' => round($rankAvg['leader'] * $team['leaders'] * self::SELL_MULTIPLIER, 2),
                ];
            }
            if ($team['members'] > 0) {
                $lineItems[] = [
                    'kind' => 'resource',
                    'label' => 'Engineer (Senior)',
                    'quantity' => $team['members'],
                    'cost' => round($rankAvg['member'], 2),
                    'amount' => round($rankAvg['member'] * $team['members'] * self::SELL_MULTIPLIER, 2),
                ];
            }
            ...
            Invoice::create([
                ...
                'amount' => $row['amount'],
                'tax' => 0,
                'paid_amount' => $isPaid ? $row['amount'] : 0,
                'status' => $row['status'],
                'paid_at' => $paid,
                'issued_at' => $isPaid ? $issue : null,
                'billing_period_label' => 'Fee for '.$billingMonth->format('M Y'),
                'line_items' => $lineItems,
                'memo' => 'Monthly service fee for '.$billingMonth->format('F Y').'.',
    ...
    private function createMonthlyTimeEntriesForTeam(Tenant $tenant, Project $project, User $admin, array $team, Carbon $start, int $projectMonths): void
    {
        ...
        foreach ($team as $member) {
            ...
            $startOffset = $member['start_offset'] ?? 0;
            $memberStart = $start->copy()->addMonths($startOffset);
